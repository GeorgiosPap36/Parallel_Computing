...
void Get_args(int argc, char* argv[], int* threads, int* r, int* c)  {
...
int Check_correctness(double* C, double* C_correct, int rows){
    for (int i = 0; i < rows; i++) {
        for (int j = 0; j < rows; j++){
            if (abs(C[i*rows + j] - C_correct[i*rows + j]) > 0.0000000000001){
...
__global__ void Mult_mats(int local_rows, int rows, int cols, double* device_A, double* device_B, double* device_C){

    for (int i = threadIdx.x * local_rows; i < (threadIdx.x + 1) * local_rows; i++) {
        for (int j = 0; j < rows; j++) {
            device_C[i*rows + j] = 0;

            for (int k = 0; k < cols; k++) {
                device_C[i*rows + j] += device_A[i*cols + k] * device_B[k*rows + j];
            }
        }
    }
}
...
int main(int argc, char* argv[]) {
    int thread_count, rows, cols, local_rows;
    ...
    Get_args(argc, argv, &thread_count, &rows, &cols);
    local_rows = rows/thread_count;
    ...
    Mult_mats <<<1, thread_count>>>(local_rows, rows, cols, device_A, device_B, device_C); 
